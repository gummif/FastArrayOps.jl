...
calc(::BM_BLAS, ::FUNT, x, y) = begin
    n = length(x)
    BLAS.axpy!(n, 1.0, y, 1, x, 1)
end
calc(::BM_Broadcast, ::FUNT, x, y) = broadcast!(+, x, x, y)
calc(::BM_Forloop, ::FUNT, x, y) = addarrloop1(x, y)
function addarrloop1(x::Array, y::Array)
    @inbounds for i=1:length(x)
        x[i] += y[i]
    end
    return x
end
calc(::BM_FAO, ::FUNT, x, y) = unsafe_fast_add!(x, 1, y, length(x))
